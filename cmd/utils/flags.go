...
var (
	...
	VecnoFlag = &cli.BoolFlag{
		Name:     "vecno",
		Usage:    "Vecno Network mainnet: pre-configured Vecno Network mainnet",
		Category: flags.EthCategory,
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.IsSet(BootnodesFlag.Name) {
		...
		case ctx.Bool(VecnoFlag.Name):
			urls = params.VecnoBootnodes
...
func dataDirPathForCtxChainConfig(ctx *cli.Context, baseDataDirPath string) string {
	...
	case ctx.Bool(VecnoFlag.Name):
		return filepath.Join(baseDataDirPath, "vecno")
	case ctx.Bool(HoleskyFlag.Name):
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, DeveloperPoWFlag, GoerliFlag, SepoliaFlag, ClassicFlag, VecnoFlag, HoleskyFlag)
...
func genesisForCtxChainConfig(ctx *cli.Context) *genesisT.Genesis {
	...
	case ctx.Bool(VecnoFlag.Name):
		genesis = params.DefaultVecnoGenesisBlock()
	case ctx.Bool(HoleskyFlag.Name):
